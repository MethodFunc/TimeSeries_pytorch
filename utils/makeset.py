...
def basicwindowmaker(features, targets, start_index, end_index, window_size):
    if isinstance(features, pd.core.frame.DataFrame) or isinstance(features, pd.core.series.Series):
...
class DataMaker(Dataset):
    ...
    def sliding_window(self):
        if isinstance(self.x_data, pd.core.frame.DataFrame) or isinstance(self.x_data, pd.core.series.Series):
            self.x_data = self.x_data.values

        if isinstance(self.y_data, pd.core.frame.DataFrame) or isinstance(self.y_data, pd.core.series.Series):
            self.y_data = self.y_data.values

        self.x_data = self.x_data[np.arange(self.window_size)[None, :] + np.arange(self.x_data.shape[0] -
